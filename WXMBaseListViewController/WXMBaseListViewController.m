...
- (void)rac_initializeRequest {
    [self.listViewModel.requestCommand.executionSignals.switchToLatest subscribeNext:^(id x) {
        NSInteger code = [x integerValue];
        if (code != WXRequestTypeFail) [self.mainTableView reloadData];
        [self endRefreshLoading];
        [self setDefaultInterface];
        NSLog(@"%@",self.listViewModel.dataSource);
    }];
}
/** 缓存 */
- (void)rac_initializeCache {
    [self.listViewModel.cacheCommand.executionSignals.switchToLatest subscribeNext:^(id x) {
        [self judgeListViewCache];
    }];
    [self.listViewModel.cacheCommand execute:nil];
}
/** 判断是否有缓存 */
- (void)judgeListViewCache {
    if (self.listViewModel.existCache == WXMExistCacheTypeNone) {
        self.hideErrorProgressHUD = YES;
        self.mainTableView.mj_footer.hidden = YES;
        [self.mainTableView showErrorStatusViewWithType:WXMErrorStatusTypeNorecord];
    } else if (self.listViewModel.existCache == WXMExistCacheTypeExistCache) {
...
/** 上下拉 */
- (void)refreshHeader {
    if (self.listViewModel.refreType == WXMRefreshHeader) return;
    self.mainTableView.scrollEnabled = NO;
    [self.listViewModel.requestCommand execute:@(WXMRefreshHeader)];
}
- (void)refreshFoot {
    if (self.listViewModel.refreType == WXMRefreshFoot) return;
    if (self.listViewModel.refreType == WXMRefreshHeader) {
        [self.mainTableView.mj_footer endRefreshing];
        NSLog(@"正在下拉刷新...");
    } else {
        [self.listViewModel.requestCommand execute:@(WXMRefreshFoot)];
    }
}
/** 结束刷新 */
- (void)endRefreshLoading {
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(.1f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        self.mainTableView.mj_footer.hidden = (self.listViewModel.dataSource.count == 0);
    });
...
- (void)setDefaultInterface {
    ...
    if (self.showErrorType == WXMShowErrorType_full) {
        ...
        if (self.listViewModel.refreType == WXMRefreshFoot) {
            self.mainTableView.mj_footer.hidden = NO;
            [self.mainTableView showErrorStatusViewWithType:WXMErrorStatusTypeNormal];
        } else {
            WXMErrorStatusType types = WXMErrorStatusTypeNormal;
            if (self.listViewModel.dataSource.count == 0) types = WXMErrorStatusTypeNorecord;
            if (self.listViewModel.dataSource.count == 0) self.hideErrorProgressHUD = YES;
...
- (NSInteger)numberOfSectionsInTableView:(UITableView *)tableView {
    if (_isGrouped) return self.listViewModel.dataSource.count;
    return 1;
}
...
- (__kindof WXMBaseListViewModel *)listViewModel {
    if (!_listViewModel) _listViewModel = [WXMBaseListViewModel wxmListWithViewController:self];
